...
- app.R is a shiny app that shows the results of the analysis
...
mortg_vars=loanamort(r=.03, #mortgage interest 3%
                     bal0=250000, #mortgaae balance
                     n=30, # 30 year amortization 
                     freq=12,
                     apr=TRUE)  


#creates all the months afterwards as time series
mort_dates=start_date+months(0:360) #total 30yr * 12mths = 360mths
mortg_bal=xts(c(250000,mortg_vars$bal), #mortgage balance
              mort_dates)
mortg_int=xts(c(0,-mortg_vars$int),mort_dates) #mortgage interest
mortg_pmt=xts(c(0,rep(-mortg_vars$pmt,360)),mort_dates) #mortgage payment
```
...
renovation=xts(rep((-renovation_amt/6),6), #negative because it is a cost
               start_date+months(1:6)) #renovation takes place for 6 mths
```

## Lease it

```{r}
#rent is 20k per year, increases 4% per year
...
```{r}
show_annual=function(xtsmat,nyear=6,isbs=FALSE) { #xtsmat is the dataset
  xtsmat=apply(xtsmat,2,nona) # get rid of NAs
  df=as.data.frame(xtsmat) # make xtsmat a dataframe
  xtsdates=as.Date(rownames(df)) # convert date column into date type
  rownames(df)=NULL # remove rownames
  years=year(xtsdates) # set variable years
  year1=years[1] # get the first year
  lastyear=tail(years,1) # get the last year
  yearlabel=year1:lastyear 
  if (isbs) { #isbs by default is FALSE
    #AGGREGATES BY YEARS
    dfy=aggregate(df,by=list(years),FUN=lastinvec) #compute summary statistics 
    # of the data subsets               # lastinvec = last element in vector
    #choose the last vector by years
    } 
  else {
    dfy=aggregate(df,by=list(years),FUN=sum) #sum by years
  }
...
#join all these lists together to get on overview of what payments
#are going out on which date
total_opcf=xts(rowSums(opcfmat,na.rm=TRUE),index(opcfmat)) #Sum up by row
